...
const { Akane, startSession, endSession, queryAkaneAPI } = require('./database/lib/akane-ai.js')
...
case 'autoai': {
  ...
  if (status === 'on') {
    const result = await startSession(m);
    reply(result);
  } else if (status === 'off') {
    const result = await endSession(m);
    reply(result);
  } else {
    reply(`Status tidak valid. Gunakan *on* atau *off*.`);
  }
    }
  break;
...
default:
  ...
  if (Akane.Chat && Akane.Chat[m.chat]) {
    if (budy.trim() && !budy.startsWith('.autoai')) {
      if (m.fromMe || handled) return; 
      const aiResponse = await queryAkaneAPI(budy);
