...
const { imageToWebp, imageToWebp3, videoToWebp, writeExifImg, writeExifImgAV, writeExifVid } = require('./database/lib/exif')
...
if(usePairingCode && !OjiOffc.authState.creds.registered) {
		const phoneNumber = await question(`${chalk.red.bold('⚪  Masukan Nomor WhatsApp Awalan 62:')}\n`);
...
OjiOffc.ev.on('messages.upsert', async (chatUpdate) => {
    try {
        ...
        const pushname = m.pushName || m.key.remoteJid;
        const from = m.key.remoteJid || '';
        const body = m.message.conversation || m.message.extendedTextMessage?.text || '';
        const botname = global.botname
        const randomcolor = listcolor[Math.floor(Math.random() * listcolor.length)];

        console.log(chalk.white.white.bold( [ botname ]), color(`⨠ Pesan`, `${randomcolor}`), color(`Dari`, `${randomcolor}`), color(`${pushname}`, `${randomcolor}`), color(` :`, `${randomcolor}`), color(`${body}`, `white`))
...
return await ore.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })}
//=================================================//
    ...
    try {
      ...
      for (let num of participants) {
        ...
        if (anu.action == 'add') {
          ...
          let txtLeft = `Bye @${num.split("@")[0]} 👋\nDari Group${metadata.subject}, Semoga Tenang Di Alam Sana`
          OjiOffc.sendMessage(anu.id, {
            document: fs.readFileSync('./package.json'),
            thumbnailUrl: ppuser,
            mimetype: 'application/pdf',
            fileLength: 99999,
            pageCount: '100',
            fileName: `Semoga Tenang Di Sana`,
            caption: txtLeft,
            contextInfo: {
              externalAdReply: {
                showAdAttribution: true,
...
console.log(`Berhasil Terhubung Ke ${global.botname}` )
}
